...
void CEngineSettingsWindow::draw()
{
    ...
    ImGui::SliderFloat("Camera Speed", &f0, 0.0f, 100.0f, "Speed = %.3f");
    mCamera.setMoveSpeed(f0);

    float f1 = mCamera.getFov();
    ImGui::SliderFloat("Camera FoV", &f1, 0.0f, 360.0f, "FoV = %.3f");
    mCamera.setFov(f1);

    glm::vec2 nf = mCamera.getNearAndFar();
    ImGui::SliderFloat("Camera NEAR", &nf.x, 0.00001f, 100.0f, "Near = %.3f");
