...
// https://github.com/SebLague/Procedural-Planets/blob/6b551429541d988e00ca9e509450e51e1f2f6f12/Procedural
// Planet E04/Assets/Noise.cs
...
constexpr std::array Grad3 = {glm::ivec3(1, 1, 0),   glm::ivec3(-1, 1, 0),  glm::ivec3(1, -1, 0),
                              glm::ivec3(-1, -1, 0), glm::ivec3(1, 0, 1),   glm::ivec3(-1, 0, 1),
                              glm::ivec3(1, 0, -1),  glm::ivec3(-1, 0, -1), glm::ivec3(0, 1, 1),
...
private:
    std::array<int, randomSize * 2> mRandom;
    double dot(const glm::ivec3& g, double x, double y, double z);
...
void Noise::randomize(unsigned int seed)
{
    if (seed)
    {
        USeed u{seed};
        ...
        {
            mRandom[i] = source[i] ^ u.c[0];
            mRandom[i] ^= u.c[1];
            mRandom[i] ^= u.c[2];
            mRandom[i] ^= u.c[3];

            mRandom[i + randomSize] = mRandom[i];
        }
    }
    else
    {
        ...
        {
            mRandom[i + randomSize] = mRandom[i] = source[i];
        }
    }
}
...
float Noise::evaluate(glm::vec3 point)
{
    ...
    {
        t0 *= t0;
        int gi0 = mRandom[ii + mRandom[jj + mRandom[kk]]] % 12;
    ...
    {
        t1 *= t1;
        int gi1 = mRandom[ii + i1 + mRandom[jj + j1 + mRandom[kk + k1]]] % 12;
    ...
    {
        t2 *= t2;
        int gi2 = mRandom[ii + i2 + mRandom[jj + j2 + mRandom[kk + k2]]] % 12;
    ...
    {
        t3 *= t3;
        int gi3 = mRandom[ii + 1 + mRandom[jj + 1 + mRandom[kk + 1]]] % 12;
...
double Noise::dot(const glm::ivec3& g, double x, double y, double z)
{
